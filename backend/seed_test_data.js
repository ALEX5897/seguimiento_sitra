...
async function seedTestData() {
  ...
  console.log('📊 Seed de datos de prueba - Solo Reasignados\n');
  ...
  try {
    // 1. Desactivar FK constraints
    console.log('\n⚙️  Desactivando constraints...');
    await conn.query('SET FOREIGN_KEY_CHECKS = 0');
    console.log('✓ FK constraints desactivadas');

    // 2. Limpiar tabla reasignados
    console.log('\n🧹 Limpiando tabla reasignados...');
    ...
    // 4. Insertar datos de prueba - 2 de cada caso
    console.log('\n📄 Insertando datos de prueba (2 de cada caso)...\n');
    ...
    const [result] = await conn.query('SELECT COUNT(*) as total FROM reasignados');
    const totalReasignados = result[0].total;
    ...
    console.log(`✓ Total reasignados: ${totalReasignados}`);
    console.log('\nPor estado:');
    byState.forEach(row => {
    ...
    console.log('\n📝 Se cargaron 18 registros de prueba (2 de cada caso)');
